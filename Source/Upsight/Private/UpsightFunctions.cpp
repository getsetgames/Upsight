...
{
    UE_LOG(LogUpsight, Log, TEXT("UpsightFunctions - billboard:didReceiveReward: - scope:%s, reward:%s, quantity:%d, signatureData:%s"), *FString([aBillboard.scope autorelease]), *FString(aPurchase.productIdentifier), aPurchase.quantity, *FString([NSString stringWithCString:(const char *)[aPurchase.signatureData bytes] encoding:NSUTF8StringEncoding]));
